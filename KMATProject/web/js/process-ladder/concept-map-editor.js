...
document.onload = (function(d3) 
{
	...
	var GraphCreator = function(svg, nodes, edges, ladder) 
	{
	   ...
	   //gradient on normal node and node hover
		...
		radialGradientSelectNode.append("stop")
			...
			.attr("stop-color", "rgba(89,148,202,1)");
                ...
                //Gradients for composition ladder
                
                var filterc = defs.append("filter")
						.attr("id", "drop-shadow-composition");
		filterc.append("feGaussianBlur")
			.attr("in", "SourceAlpha")
			.attr("stdDeviation", 3)
			.attr("result", "blur");

		filterc.append("feOffset")
			.attr("in", "blur")
			.attr("dx", 3)
			.attr("dy", 4)
			.attr("result", "offsetBlur");

		var feMergec = filterc.append("feMerge");

		feMergec.append("feMergeNode")
			.attr("in", "offsetBlur");
		feMergec.append("feMergeNode")
			.attr("in", "SourceGraphic");
	   ...
	   //gradient on normal node and node hover
		var radialGradientNormalNodec = defs.append("radialGradient")
											.attr("id", "radial-gradient-normal-composition")
											.attr("cx", "0.5")
											.attr("cy", "0.5")
											.attr("r", "0.5")
											.attr("fx", "0.25")
											.attr("fy", "0.25");
		radialGradientNormalNodec.append("stop")
				.attr("offset", "0")
				.attr("stop-color", "#F6FBFF");
				//.attr("stop-color", "#F6FBFF");
		radialGradientNormalNodec.append("stop")
		...
		//gradient on node select
		var radialGradientSelectNodec = defs.append("radialGradient")
											.attr("id", "radial-gradient-select-composition")
											.attr("cx", "0.5")
											.attr("cy", "0.5")
											.attr("r", "0.5")
											.attr("fx", "0.25")
		...
		radialGradientSelectNodec.append("stop")
			.attr("offset", "0")
			.attr("stop-color", "rgba(248,255,232,1)");
        ...
        //Gradients for Concept Nodes
        
        var filtern = defs.append("filter")
						.attr("id", "drop-shadow-concept");
		filtern.append("feGaussianBlur")
			.attr("in", "SourceAlpha")
			.attr("stdDeviation", 3)
			.attr("result", "blur");

		filtern.append("feOffset")
			.attr("in", "blur")
			.attr("dx", 3)
			.attr("dy", 4)
			.attr("result", "offsetBlur");

		var feMergen = filtern.append("feMerge");

		feMergen.append("feMergeNode")
			.attr("in", "offsetBlur");
		feMergen.append("feMergeNode")
			.attr("in", "SourceGraphic");
	   ...
	   //gradient on normal node and node hover
		var radialGradientNormalNoden = defs.append("radialGradient")
											.attr("id", "radial-gradient-normal-concept")
											.attr("cx", "0.5")
											.attr("cy", "0.5")
											.attr("r", "0.5")
											.attr("fx", "0.25")
											.attr("fy", "0.25");
		radialGradientNormalNoden.append("stop")
				.attr("offset", "0")
				.attr("stop-color", "#F6FBFF");
		radialGradientNormalNoden.append("stop")
				.attr("offset", "100%")
				.attr("stop-color", "#9EED73");
	  
	  
		//gradient on node select
		var radialGradientSelectNoden = defs.append("radialGradient")
											.attr("id", "radial-gradient-select-concept")
											.attr("cx", "0.5")
											.attr("cy", "0.5")
											.attr("r", "0.5")
											.attr("fx", "0.25")
		...
		radialGradientSelectNoden.append("stop")
			.attr("offset", "0")
			.attr("stop-color", "rgba(248,255,232,1)");
		...
		svg.on("focusout", function()
			{
				...
				{
					thisGraph.modalJustClosed = false;
					thisGraph.selectedGraphCircle.classed("selectedC circle", false);
                                        //thisGraph.selectedGraphCircle.classed(thisGraph.state.selectedNode.classforSelection+" circle", true);
				}
		...
		d3.select("#center-input").on("click", function()
			{
                ...
                //on save, check if all blues/oranges are already processes/compositions
		d3.select("#save-input").on("click", function() 
			{
				...
				else
				{
					...
					xhttp.onload = function() 
					{
						if(xhttp.status === 200) 
						{
                                                    if (xhttp.responseText.indexOf("does not exist") >= 0)
                                                    {
                                                        //composition or process ladder does not exist
					...
					var action = "action=save-concept-map&newLadder=" + ladder;
					xhttp.send(action);
				}
			});
		...
		{
			if(isNodeSelected && thisGraph.isNodeSelected)
			{
			...
			else if(isEdgeSelected && thisGraph.isEdgeSelected)
			{
...
//TODO:
	...
	GraphCreator.prototype.replaceSelectEdge = function(d3Path, edgeData)
	{
		var thisGraph = this;
                ...
                //d3Path.classed(thisGraph.state.selectedNode.classforSelection, true);
	...
	// mouseup on main svg
        //check if click-ctrl or click-alt or click-shift to make a node
	GraphCreator.prototype.svgMouseUp = function()
	{
		...
		{
                        thisGraph.consts.circleGClass = "conceptC";
                        thisGraph.consts.selectedClass = "selectedC";
			...
			txtNode.focus();
                ...
                //Process Node
                else if(state.graphMouseDown && d3.event.altKey)
		{
                    //alert("alt+click");
                        thisGraph.consts.circleGClass = "conceptP";
                        thisGraph.consts.selectedClass = "selectedP";
			thisGraph.numTotalNodes++;
			var xycoords = d3.mouse(thisGraph.svgG.node()),
				d = {
						id: thisGraph.idct++,
		...
		else if(state.graphMouseDown && d3.event.shiftKey)
		{
			// clicked not dragged from svg
                        thisGraph.consts.circleGClass = "conceptG";
                        thisGraph.consts.selectedClass = "selectedG";
                        thisGraph.numTotalNodes++;
			var xycoords = d3.mouse(thisGraph.svgG.node()),
				d = {
						id: thisGraph.idct++,
						...
						description: "",
						title: "New Concept " + thisGraph.numTotalNodes,
						tools: [],
						resources: [],
						users: [],
						x: xycoords[0], 
        ...
        //do not open if node type is process or composition
...
//TODO: ask if the user wants to delete selected edges as well?
	...
	GraphCreator.prototype.updateGraph = function()
	{
		...
		paths.style('marker-end', 'url(#end-arrow)')
                        //.classed(thisGraph.state.selectedNode.classforSelection, function(d)
				...
				if(!d3.event.shiftKey && !thisGraph.state.shiftNodeDrag)
				{
					...
					if(thisGraph.state.selectedEdge)
					{
						thisGraph.removeSelectFromEdge();
                                        ...
                                        //d3.select(this).classed(thisGraph.consts.selectedClass+" circle", true);
					if(thisGraph.state.selectedNode)
					{
                                                d3.select(this).classed(thisGraph.state.selectedNode.classforSelection, true);
						thisGraph.removeSelectFromNode();
					}
		...
		//newGs.classed(consts.circleGClass, true)
			...
			.on("dblclick", function(d)
			{
				...
				if(!thisGraph.state.justDragged && !d3.event.shiftKey && !thisGraph.state.shiftNodeDrag)
				{
					...
					if(thisGraph.state.selectedNode)
					{
                                                d3.select(this).classed(thisGraph.state.selectedNode.classforSelection+" circle", true);
						thisGraph.removeSelectFromNode();
					}
