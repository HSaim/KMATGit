...
{
	if(accordionId === "accordion2-panel")
	{
...
function addNewItemView(panelId, item, itemId)
{
	var accordionPanel = document.getElementById(panelId);
	//keep track of number of items (except for the default last item)
	if(isNaN(accordionPanel.numListElements) || accordionPanel.numListElements === null)
...
function onLoadModalView(callingNodeId, linkedObjectsList)
{
	...
	for(var i = 0; i < linkedObjectsList.length; i++)
	{
            addNewItemView(callingNodeId, linkedObjectsList[i], i);
	}
        searchResultResources = linkedObjectsList;
}
	...
	if(isNodeSelected)
	{
		...
		else
		{
			...
			if(selectedNode.users === null)
				selectedNode.users = [];
//                        allResources = document.getElementById("all-resources").innerText;
//                        searchResultResources = allResources.split(" ,");
//			for(var i = 0; i < searchResultResources.length; i++)
//			{
//                            selectedNode.resources.push(searchResultResources[i]);
//			}
                        allResources = document.getElementsByClassName("accordion-inner-panel accordion-inner-panel-accordion2-panel");
                        for(var i = 0; i < allResources.length; i++)
			{
                            selectedNode.resources.push(allResources[i].innerText.trim());
			}
		}
	}
...
function closeInnerModal()
{
	...
	if(callingNodeId === "accordion2-panel")
	{
...
function addSearchItem(el)
{
	...
	if(el.parentId === "accordion2-panel")
	{
